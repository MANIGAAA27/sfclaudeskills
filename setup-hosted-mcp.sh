#!/usr/bin/env bash
# Auto-setup for Salesforce Hosted MCP (Beta) for Claude Code: prereqs, optional scratch org, generate .mcp.json.
...
# Env: SALESFORCE_MCP_SERVER (default platform/sobject-all), SALESFORCE_MCP_SANDBOX (non-empty = sandbox).
# Claude Code uses .mcp.json (project scope). Use /mcp in Claude Code to authenticate with your External Client App.
set -euo pipefail
...
# Claude Code .mcp.json: SERVER_PATH only (OAuth via /mcp in Claude Code)
...
TEMPLATE="$TOOLKIT_DIR/templates/mcp-hosted.json"
if [[ ! -f "$TEMPLATE" ]]; then
  echo "Error: Template not found: $TEMPLATE"
  exit 1
fi

sed -e "s|SERVER_PATH|$SERVER_PATH|" "$TEMPLATE" > "$PROJECT_ROOT/.mcp.json"
echo "Wrote $PROJECT_ROOT/.mcp.json (server path: $SERVER_PATH)"
echo ""
...
echo "3. In Claude Code, run /mcp to authenticate with your External Client App (OAuth)."
echo "4. Test the connection in Claude Code."
echo ""
